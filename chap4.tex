...
These effects are more clearly seen in the trends of Figure \ref{process_rate}.  This figure shows the rate at which each implementation is able to process neutrons vs the number of active neutrons left in the batch.  The figure has been smoothed with a window 11 data points wide, and linearly interpolated where the timings were too close together and gave Inf or NaN results for processing rates.  The remapping implementation is slower at large numbers of active neutrons since accessing the entire dataset generally yields active data anyway.  Conversely, the remapping implementation is faster when there few active neutrons left in the batch and accessing the entire grid generally yields terminated data.  The tradeoff in the remapping implementation is that the sort adds overhead when there are many active particles, but eliminating completed data and thus reducing the grid size makes it much more efficient when there are few active neutrons left.  Since the radix sort is a very efficient operation, the addition cost is small in the (about 30\% slower), but the benefit at small neutron numbers pays for this (about 3x faster).   In the non-remapping implementation, the addition cost at low numbers of active neutrons is due to launching a large grid where most of the blocks access terminated data and simply return without doing work.  Compute cycles and, more importantly, loads from global memory are wasted simply to check if a neutron is terminated already or not.

It therefore makes sense that the remapping implementation performs much better when the simulation spends significant time in the sub-300k active neutrons region on the processing rate curve (about where the lines cross).  In the Jezebel, homogenized block, pin cell, and assembly benchmarks, 90\%, 77\%, 91\%, and 96\% of the transport iterations occur in this region, respectively.  It also makes sense that the relative number of iterations  spent in the regions is vaguely constant.  The particles die out exponentially, and it takes roughly the same number of transport iterations to go from $10^7$ to $10^6$ active neutrons as it does to go from $10^2$ to $10^1$ active neutrons.
...
Compaction         &  3.8      &        7.3                  &   11.2   &  20.1        \\ \hline
 Yield Reduction  &   1.5     &        2.6                 &  4.0    &    7.3     \\ \hline
 Yield Rebase     &    0.0     &       0.0                   &   0.0  &    0.0    \\ \hline
 Yield Prefix Sum &   1.4    &       2.5                    &  4.0   &    7.2    \\ \hline
...
compaction in nonremapping also include the cost of flipping the done flags to make an ``is valid'' vector

not only to the specific reaction kernels benefit, but the main time savings is in the global functions being able to reduce the size of their launches ( trace,  rng seeds (might be unecesary), scans, reductions, sorts) 

divergence from reactions seems to be minimal since most of the time is spent querying geometry and cross sections, not processing the reactions,  remapping helps, expecially since radix is done in-place rather than having to be done and copied, but its main benefit is by eliminating stale access, and therefore grid size.
