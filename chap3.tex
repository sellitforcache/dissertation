...
The outer loop, shown in Figure \ref{warp_outer_loop}, uses the result from the inner loop to set up the next batch of neutrons need

\begin{figure}[h!] 
\centering
\includegraphics[width=0.9\textwidth]{graphics/warp_outer_loop.eps}
\caption{WARP outer transport loop that is executed in between neutron batches. \label{warp_outer_loop} }
\end{figure}

Fixed source runs do not have an outer loop since the source is not dependent on the flux.
...
Once this is known, a kernel is launched to do a search on the unionized energy grid to determine the index $i$ which satisfies $E_i<E<E_{i+1}$.
...
?? O(1)	
?? Linearize	so	every	bin	is	hit	at	least	
once	by	a	line	
?? dx	=	smallest	di?erence	in	grid	
?? 20MeV/dx	~	1014		LLL	
?? Very	high	order	polynomials?	
?? Accuracy?	
?? Time?	
?? Use	line	or	polynomial	as	iniFal	
guess	for	binary	search	to	decrease	
loads?	
?? Is	there	a	way	to	e?ciently	hash	a	
range	of	?oats	to	an	integer?

binary search here? more logN?  quad tree?  approximate function for initial guess?

\subsection{Pseudo-Random Numbers}

use of rejection sampling disqualifies pre computation of random number, plus it is very bandwith intensive to do so!

using lcrng keeps global access down and improves performance greatly, but curand is used outside of cycle to make sure no correlated numbers are used as intermediate seeds.

\subsection{Macroscopic}

From here the macroscopic kernel is launched which computes the total macroscopic cross section for the material, samples the interaction distance, which isotope the neutron interacts with, moves the neutron to the nearer of the interaction/intersection distance, and sets the neutrons reaction number to the resample flag if the intersection is closer.

does things related to the macroscopic cross section whole material, samples isotope
