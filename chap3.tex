...
Traces were also done in a cell containing no objects, and rates were on the order of ten times faster.  This case was simply performed as a control, since a completely homogenized domain would eliminate the need for a geometry routine in the first place.  This study was done in order to show which OptiX configuration would produce the best results in a reactor-like geometry.  Also, traces were done where the direction of every particle was in the negative $z$ direction, since this is the direction with the fewest number of surfaces between the source particles and the boundary cell.  Performance was increased marginally, but this can only be taken advantage in geomtrries where there is a direction of least heterogeneity (like lattices).
...
single vs double precision, justification physics wise and of course cost and performance wise.

At this point, all the geometry routines are set and only the energy routines and the 
...
\begin{figure}[h!] 
\centering
\includegraphics[width=0.5\textwidth]{graphics/unionized_pointers.eps}
