...
Fission releases a substantial amount of energy, which is why nuclear fuel is considered very energy-dense.  $^{235}$U, for example, releases a total of 192.9$\pm$0.5 MeV per fission \cite{duderstadt}.  Compared to chemical energy sources where the energy released per reaction is on the order of 1 eV.  Fission energy yield is 8 orders of magnitude larger!  
% You already talked about this in an earlier section. You can probably condense one or the other of them.
...
$\nu_\mathrm{T}$ is shown at one temperature since it has very weak temperature dependence for most energies.   In fact, $\nu_\mathrm{T}$ is nearly constant from low energies up to energies in the MeV range, where it increases sharply due to the energy the incident neutron provides being sufficient to eject a bound neutron.  

Figure \ref{xs_fission_only} shows the total fission cross sections for two \emph{fissile} isotopes, $^{235}$U and $^{239}$Pu, and one \emph{fertile} isotope, $^{238}$U.  Fissile isotopes are those where neutrons at any energy can induce fission. Fertile isotopes are those that produce a fissile isotope after absorbing a neutron.
% I think you use the term fissile before this, so you might want to define them earlier. 
The figure shows that  $^{238}$U has a fission cross section, but it isn't significant until above 1 MeV.  For $^{238}$U, fission is a threshold reaction.% You might want to mention why it exists at very low energies since this seems inconsistent.
  Simply absorbing a neutron does not provide enough energy to split the nucleus.  Additional energy is required, which can be provided in the form of an incident neutron's kinetic energy.  $^{238}$U isn't fissile, but it can be a significant contributor of fission reactions in reactors where the neutron population is \emph{fast}, i.e.\ mainly high-energy.  As mentioned before, $^{238}$U is considered fertile because it is converted to the fissile isotope $^{239}$Pu \cite{duderstadt}.  
...
% I don't think you need the assumption because the temperature in the fuel will rise more rapidly than any coolant mechanism can respond, which is why the doppler effect always applies at least in an instantaeous sense.
...
This is effectively the loss term for a beam in the $\hat{x}$ direction, and if we set it as such, we recover the linear attenuation expression shown in \eqref{recover_beam}, which is equivalent to \eqref{pop_diff}.  
...
OpenMC is also under development to run on GPUs by NVIDIA themselves.  Progress was presented at the 2014 GPU Technology Conference (GTC) \cite{scudiero}.
