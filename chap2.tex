...
Applying this to distribution of mean values inn \eqref{sum_var}, an expression for the variance of the mean in terms of the variance of the sample be found.  

\begin{equation}
\label{sum_var}
Var(\bar{X}_i) = Var\left(\frac{1}{N}\sum_{i=0}^N x_i \right) = \frac{1}{N^2} \sum_{i=0}^N Var(x_i) = \frac{\sigma_N^2}{N} = \frac{1}{(N-1)} \left( \frac{1}{N}\sum_i^N x_i^2-\bar{X}_N^2 \right)
\end{equation}

It should be noted that \eqref{sum_var} implies that the standard deviation of the mean scales as $1/\sqrt{N}$ in the worst case.  This is a blessing and a burden in that it means any calculation will variance of the mean will go to zero, or the sample mean will converge to the true mean, as long it is run long enough and enough samples are collected, but that it will converge as $1/\sqrt{N}$, which is slow.
...
The direct inversion method relies and being able to analytically integrate the probability distribution function (PDF) to create a cumulative distribution function (CDF) and being able to invert it.  The first step is to integrate the PDF to find the CDF, as shown in \eqref{CDF}.
...
\begin{equation}
\label{CDF}
CDF(x) =\xi \qquad \Rightarrow \qquad x = CDF^{-1}(\xi)
\end{equation}

\begin{figure}[h!] 
  \label{direct_samp}
  \centering
    \includegraphics[width=0.8\textwidth]{graphics/direct_samp.eps}
     \caption{Sampling from a CDF.}
\end{figure}
