...
In the 1991 paper, a vectorized approach is applied to the collision probability method (CPM), which is a deterministic way to solve the integral form of the NTE \cite{vujic_vector}.  The paper does not 
...
OpenMC is a ``Monte Carlo particle transport simulation code focused on neutron criticality calculations ... [that] was originally developed by members of the Computational Reactor Physics Group at the Massachusetts Institute of Technology starting in 2011'' \cite{openmc}.  It is a new code that aims to be an open testing platform for novel Monte Carlo algorithms.  It is cited often in the work due it its freely available source code and thorough documentation.  OpenMC is also being developed by NVIDIA to run on GPUs.  Progress was presented at the 2014 GPU Technology Conference (GTC), but mainly consisted of accelerating xsbench, OpenMC's synthetic cross section processing benchmark tool \cite{scudiero,openmc}.  % I think this is the first time you mention this code, so if you're going to talk about it you should tell us something about it.   Yup.

WARP aims to be much more general and accurate than all previous codes while employing efficient parallel algorithms in order to have a completely GPU-accelerated code.  WARP will read ACE-formatted data, perform all reaction types as prescribed by them, use a Serpent-like unionized energy grid to regularize data access, use an event-based transport algorithm with parallelized operations for sorts and sums, use OptiX for general 3D geometry representation (without explicit nesting), use a SOA for neutron history data, and perform \emph{all} operations on the GPU unless strictly forbidden for some reason (the benefit of which will be discussed later).% you might want to change that to a bulleted list to highlight what is new and useful about your code. A big part of your job is explaining why your work is a new contribution.

To recap its novel features, WARP...
\begin{itemize}
  \item Uses an event-based transport algorithm to conduct continuous energy Monte Carlo neutron transport on a GPU
  \item Uses a vector of neutron references to remap data access on-the-fly, therefore minimizing thread divergence and simultaneously eliminate completed histories
  \item Uses nuclear data loaded from ACE formatted files
  \item Treats interactions exactly as specified by the nuclear data files
  \item Uses the NVIDIA OptiX ray tracing framework for general 3D geometry representation 
  \item Can produced results (multiplication factors, fission distributions, flux sprectra) comparable to production Monte Carlo neutron transport codes
\end{itemize}
