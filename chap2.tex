...
Now that the events which can happen to neutrons have been outlined, we will move to describing the neutron population itself.  Since the neutron population in reactors is large, on the order of $10^{XXX}/\mathrm{cm}^3$, and the neutrons themselves have very small radii, about $1.75\times10^{-17}$ cm, the distribution of neutrons can be treated as an continuum.  

Eventually come to a linearized version of the Boltzmann transport equation since neutrons are assumed to not interact with each other - takes advantage of the fact that the neutron density is many orders of magnitude smaller than the material density. 
...
\begin{figure}[h!] 
  \label{ang_relations}
  \centering
    \includegraphics[width=0.8\textwidth , trim= 0cm 2cm 0cm 2cm]{graphics/ang_relation.eps} % trim = l b r t
...
\begin{equation}
\label{1d_diff}
\int_A \int_{\boldsymbol{\hat{\Omega}}} \int_0^{\infty}  dE d\boldsymbol{\hat{\Omega}} dxdydz  v(E) n(\boldsymbol{\vec{r}},\boldsymbol{\hat{\Omega}},E,t) \Sigma(E) \delta(E-E_0) \delta(\boldsymbol{\hat{\Omega}}-\hat{x})  = v \Sigma A n(x)dx 
\end{equation}
...
Scattering and neutron movement have angular dependence, but reaction cross sections do not, so the reactions can be written in terms of angular flux which has been integrated over angle, or the \emph{scalar flux} or simply the \emph{flux}.  This is usually the most interesting quantity in reactor physics since the reactor power profile is directly proportional to it.

\begin{equation}
\label{scalar_flux}
\phi(\boldsymbol{\vec{r}},E,t) = \int_{\boldsymbol{\Omega}} \psi(\boldsymbol{\vec{r}},\boldsymbol{\hat{\Omega}},E,t)
\end{equation}

\begin{equation}
\label{scalar_flux_RR}
 R(\boldsymbol{\vec{r}},E,t) = \int_{\boldsymbol{\Omega}} \Sigma(E) \psi(\boldsymbol{\vec{r}},\boldsymbol{\hat{\Omega}},E,t) = \Sigma(E) \int_{\boldsymbol{\Omega}} \psi(\boldsymbol{\vec{r}},\boldsymbol{\hat{\Omega}},E,t) = \Sigma(E) \phi(\boldsymbol{\vec{r}},E,t)
 \end{equation}

\subsubsection{Scattering}

Scattering changes a neutrons angle and energy, so the scattering cross section is \emph{doubly differential} rather than being a scalar quantity.  So instead of only having energy dependence, it also depends on outgoing angle and energy, which are shown with primes in \eqref{scattering_DD_xs}.

\begin{equation}
\label{scattering_DD_xs}
\Sigma_s = \Sigma_s(E \rightarrow E^\prime,\boldsymbol{\hat{\Omega}} \rightarrow \boldsymbol{\hat{\Omega}}^\prime)
 \end{equation}

Granted, in the data, the scattering cross sections are integrated over exiting energy and angle to make a scalar quantity that can be compared against other cross sections and used to determine \emph{whether} scattering happens in a Monte Carlo simulation rather than \emph{how} it happens.


\subsubsection{Advection and Leakage}

so far all integrals, now time for some derivatives
