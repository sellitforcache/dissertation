...
Geometrical primitives in OptiX are represented by their intersection programs.  Intersection programs provide two purposes - to calculate the exact intersection points between a ray and a primitive and to provide axis-aligned bounding boxes around the primitives.  These bounding boxes must completely contain the underlying primitive, but be as small as possible for best performance.  They are queried when the acceleration structure is built over the geometry, which is discussed in the next subsection.  Their main purpose is to calculate the intersection points, which must be reported back in terms of the $t$ value.  For example, the equation for a sphere is $r^2=x^2+y^2+z^2$.  Substituting \eqref{parameterized_ray} into this equation and solving for $t$ yields \eqref{ray_sphere}.  Since a sphere is a closed surface, there are two intersection points.  The nearer of the two is reported back by the intersection program.
...
The framework by which geometric primitives are arranged into a scene is very flexible in OptiX.  Figure \ref{node_graph}
...
Although there has been much research done on Monte Carlo neutron transport, there is relatively very little about performing in on GPUs due to GPGPUs being a new technology.

differences from Nelson, look at assumptions table
	full cross sections, scattering, all reaction types, all physics
	no Q values used for inelastic!
	therefore comparisons against production codes
	data parallel (couldnÕt find any use of vectorization like he saysÉ) Ñ SOA instead of AOS type access,
	sorting
	optix, general geometry input
	unionized grid, cross section tabulation
	random number generation
	general input processing
	no nesting capability, nesting must be explicitly defined
	fixed nu
	
differences from other studies, LIU, china
	stanford guy - photons, cross sections
	cross sections, all assume constant, not realistic since memory bandwidth impact is huge on GPU
	geometry of course
