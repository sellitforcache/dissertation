...
As stated before, the developer must provide all the programs for OptiX.  These various programs are compiled to .ptx files, the paths to which are given the the OptiX API at compile time and are loaded by the executable at run time \cite{optix}.  Figure \ref{optix_flow} shows the control flow in OptiX.  The ray generation program is like a main function in the ray tracer. It initializes rays, their starting points and directions, as well as specifies the data that they report.  Once this is done, the trace is done, which traverses the geometry
