...
WARP aims to be much more general and accurate than all previous codes while employing efficient parallel algorithms in order to have a completely GPU-accelerated code.  WARP reads ACE-formatted data, performs all reaction types as prescribed by them, uses a Serpent-like unionized energy grid to regularize data access, uses an event-based transport algorithm with parallelized operations for sorts and sums, use OptiX for general 3D geometry representation (without explicit nesting), use a SOA for neutron history data, and perform \emph{all} operations on the GPU unless strictly forbidden for some reason (the benefit of which will be discussed later).% you might want to change that to a bulleted list to highlight what is new and useful about your code. A big part of your job is explaining why your work is a new contribution.
...
\begin{itemize}
  ...
  \item Uses a modified unionized energy grid data structure to minimize data traffic when performing cross section lookups
  ...
  \item Uses the CUDPP libraries to perform parallel sorts, scans, and sums on the GPU 
  \item Can produced results (multiplication factors, fission distributions, flux spectra) comparable to production Monte Carlo neutron transport codes
\end{itemize}
